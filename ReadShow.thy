...
(*Empty strings might cause problems returning True. Foldr of empty char is also 0*)
...
(*TODO: Problem with ngatives preceded by zeros. Need something in the mean time using uint function*)
value \<open>Show\<^sub>i\<^sub>n\<^sub>t (Read\<^sub>i\<^sub>n\<^sub>t (''-0'')) = ''-0''\<close>
value "Show\<^sub>i\<^sub>n\<^sub>t (Read\<^sub>i\<^sub>n\<^sub>t (''00'')) "
(* TODO
Need to address problems with leading zeros on strings.
currently making the assumption that leading zeros will return false.
This shouldnt happen semantically I think.
*)
definition 
