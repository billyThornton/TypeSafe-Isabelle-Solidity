...
(*
  TODO v.8 Add Pointers to calldata
*)
datatype LType = LStackloc Location
               | LMemloc Location
               | LStoreloc Location

(*Add mmsel  tuint here*)
...
(* Notes about method calls:
  ...
  When calling it is possible to pass a memory struct as a parameter, This is loaded into the new
  environment. If the expression then returns a pointer to a sub location in this structure then 
  the locations may not be the same. It is assumed that the struct passed in exists in the original
  memory. A check is therefore added to ensure that if a memory location is returned accessing that
  location in the loaded (modified memory) is the same as accessing it in the original memory.
  Not only must the pointer exists but the values must be the same 
  Sideffect free expressions, nothing should change in memory loaded thus the values should all be the
  same in the oriignal memory.

(if (accessStore cdloc cd = accessStore cdloc cd\<^sub>l \<and> accessStore cdloc cd \<noteq> None) then return (val, typ) else
 (if (accessStore memloc (memory st) = accessStore memloc m\<^sub>l \<and> accessStore memloc (memory st) \<noteq> None) then return (val, typ) else 
...
(*Currently stoping the loading of all storage variables but this isnt quite correct, as an internal call should load all the storage variables
...
| "load cp ((i\<^sub>p, t\<^sub>p)#pl) (ex#el) e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g =
    (case t\<^sub>p of Storage x \<Rightarrow> throw Err
        | _ \<Rightarrow> 
            case denvalue e\<^sub>v' $$ i\<^sub>p of Some x' \<Rightarrow> throw Err
            | None \<Rightarrow>
            do {
            (v, t) \<leftarrow> expr ex e\<^sub>v cd st;
            (c, m, k, e) \<leftarrow> case decl i\<^sub>p t\<^sub>p (Some (v,t)) cp cd (memory st) ((storage st) (address e\<^sub>v)) (cd', mem',  sck', e\<^sub>v') of Some (c, m, k, e) \<Rightarrow> return (c, m, k, e) 
...
next
  ...
  proof (rule allI[THEN allI, OF impI])
    ...
    next
      ...
      ultimately show "g4 \<le> g" using lift_gas[OF lift] \<open>\<not> g \<le> gc\<close> by (simp split:result.split_asm Stackvalue.split_asm Type.split_asm prod.split_asm)
    qed
  qed
next
...
next
          ...
          next
            ...
            proof (cases a)
              ...
              proof ((rule allI)+,(rule impI))
                ...
                with Pair fields have "g' \<le> g'' \<and> address ev = address e\<^sub>v'' \<and> sender ev = sender e\<^sub>v'' \<and> svalue ev = svalue e\<^sub>v''  \<and> contract ev = contract e\<^sub>v''"  using 28(3) n Pair 1 Value none by blast
          ...
          next
            ...
            proof (cases a)
              ...
              proof ((rule allI)+,(rule impI))
                ...
                by (metis (no_types, lifting) "28.hyps"(5))
                moreover from n have "g'' \<le> g" using 28(2) Calldata none by (simp add: "28.hyps"(4))
                moreover from Some fields have "address e\<^sub>v'' =  address e\<^sub>v' \<and> sender e\<^sub>v'' = sender e\<^sub>v' \<and> svalue e\<^sub>v'' = svalue e\<^sub>v' \<and> contract e\<^sub>v'' = contract e\<^sub>v'" using decl_env by auto
                ultimately show "g' \<le> g \<and> address ev = address e\<^sub>v' \<and> sender ev = sender e\<^sub>v' \<and> svalue ev = svalue e\<^sub>v' \<and> contract ev = contract e\<^sub>v' " by auto
              qed
            qed
          ...
          next
            ...
            proof (cases a)
              ...
              proof ((rule allI)+,(rule impI))
                ...
                with load_def have "load cp pl el e\<^sub>v'' cd'' k'' m'' e\<^sub>v cd st g'' = Normal ((ev, cda, k, m), g')" by simp
                with Pair fields have "g' \<le> g'' \<and> address ev = address e\<^sub>v'' \<and> sender ev = sender e\<^sub>v'' \<and> svalue ev = svalue e\<^sub>v'' \<and> contract ev = contract  e\<^sub>v''" using 28(3) n Pair 1 Memory none by (metis "28.hyps"(7))
                moreover from n have "g'' \<le> g" using 28(2) Memory none by (simp add: "28.hyps"(6))
                moreover from Some fields have "address e\<^sub>v'' =  address e\<^sub>v' \<and> sender e\<^sub>v'' = sender e\<^sub>v' \<and> svalue e\<^sub>v'' = svalue e\<^sub>v'\<and> contract e\<^sub>v'' = contract e\<^sub>v'" using decl_env by auto
                ultimately show "g' \<le> g \<and> address ev = address e\<^sub>v' \<and> sender ev = sender e\<^sub>v' \<and> svalue ev = svalue e\<^sub>v'\<and> contract ev = contract e\<^sub>v'" by auto
              qed
            qed
  ...
  next
    case (Storage x4)
    then show ?thesis using 28(1) load.psimps by simp
  qed
next
  case 29
...
next
 case (28 cp i\<^sub>p t\<^sub>p pl ex el e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g)
  show ?case
  proof(intros)
    (*cd is the source calldata for a calldata copy
        and v,t is the pointer source and the source type*)
    ...
    then have none:"denvalue e\<^sub>v' $$ i\<^sub>p = None" using load.simps(1)[of cp i\<^sub>p t\<^sub>p  pl ex  el e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g] as1 by (simp split:Type.splits option.splits)

    then obtain x g4' where a7:"expr ex e\<^sub>v cd st g = Normal (x, g4')" using as1 load.simps(1)[of cp i\<^sub>p t\<^sub>p  pl ex  el e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g] by (simp split: Type.splits result.splits prod.splits option.splits)
    ...
    then obtain c m' k' e where a15:"decl i\<^sub>p t\<^sub>p (Some (v,t)) cp cd (memory st)  (storage st (address e\<^sub>v)) (cd', mem',  sck', e\<^sub>v') = Some (c, m', k', e)"  using as1 load.simps(1) by (auto split: Type.splits result.splits prod.splits option.splits)
    then have a20:"load cp pl el e c k' m' e\<^sub>v cd st g4' = Normal ((ev, cda, k, m), g')"  using as1 load.simps(1)[of cp i\<^sub>p t\<^sub>p  pl ex  el e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g] a10 a15 by (simp split:Type.splits option.splits)
  ...
  proof(intros)
    ...
    next
      ...
      next
        ...
        have "\<forall>ev cda k m g' id x. load cp pl el  e c k' m' e\<^sub>v cd st g4' = Normal ((ev, cda, k, m), g') \<and> denvalue e $$ id = Some x \<longrightarrow> denvalue ev $$ id = Some x"
          using 28(4) Calldata none a10 a20 a7 a8 a10 a15 none False by fastforce
        then have "denvalue e $$ id = Some x \<longrightarrow> denvalue ev $$ id = Some x" using a20 by blast
        then show ?thesis using a50 by simp
      qed
    next
      ...
      next
        ...
        then have a50:" denvalue e $$ id = Some x" using a15 a20 as1 none 
          using decl_env_monotonic by presburger
        have "\<forall>ev cda k m g' id x. load cp pl el  e c k' m' e\<^sub>v cd st g4' = Normal ((ev, cda, k, m), g') \<and> denvalue e $$ id = Some x \<longrightarrow> denvalue ev $$ id = Some x"
          using 28(6) Memory a20 a7 a8 a10 a15 none False by fastforce
        then have "denvalue e $$ id = Some x \<longrightarrow> denvalue ev $$ id = Some x" using a20 by blast
        then show ?thesis using a50 by simp
      qed
    next
...
lemma load_denval_storage_remain[rule_format]:
  "\<forall>l1'  t1' g1' arr. msel c1 t1 l1 xe1 ev1 cd1 st1 g1 = Normal ((l1', t1'), g1') \<longrightarrow> True"
  "\<forall>l2' v2' t2' g2'. ssel t2 l2 xe2 ev2 cd2 st2 g2 = Normal ((l2', t2'), g2') \<longrightarrow> True"
  "\<forall>v t g4'. expr e4 ev4 cd4 st4 g4 = Normal ((v, t), g4') \<longrightarrow> True"        (*lev0 lcd0 lk lm*)

  "\<forall>ev cd k m g' id tp l. load lcp lis lxs lev0 lcd0 lk lm lev lcd lst lg = Normal ((ev, cd, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l) \<longrightarrow> (denvalue lev0) $$ id = Some (Storage tp, l) "
  "\<forall>v3' t3'  g3'. rexp l3 ev3 cd3 st3 g3 = Normal ((v3', t3'), g3') \<longrightarrow> True"
proof (induct rule: msel_ssel_expr_load_rexp.induct
    [where ?P1.0="\<lambda>c1 t1 l1 xe1 ev1 cd1 st1 g1. (\<forall>l1'  t1' g1' arr. msel c1 t1 l1 xe1 ev1 cd1 st1 g1 = Normal ((l1',  t1'), g1') \<longrightarrow> True)"
      and ?P2.0="\<lambda>t2 l2 xe2 ev2 cd2 st2 g2. (\<forall>l2' v2' t2' g2'. ssel t2 l2 xe2 ev2 cd2 st2 g2 = Normal ((l2',  t2'), g2')\<longrightarrow> True)"
      and ?P3.0="\<lambda>e4 ev4 cd4 st4 g4. (\<forall>v t g4'. expr e4 ev4 cd4 st4 g4 = Normal ((v, t), g4')\<longrightarrow> True)"
      and ?P4.0="\<lambda>lcp lis lxs lev0 lcd0 lk lm lev lcd lst lg. (\<forall>ev cd k m g' id tp l. load lcp lis lxs lev0 lcd0 lk lm lev lcd lst lg = Normal ((ev, cd, k, m), g') 
                                                                                    \<and> denvalue ev $$ id = Some (Storage tp, l) \<longrightarrow> (denvalue lev0) $$ id = Some (Storage tp, l))"
      and ?P5.0="\<lambda>l3 ev3 cd3 st3 g3. (\<forall>v3' t3' g3'. rexp l3 ev3 cd3 st3 g3 = Normal (( v3',  t3'), g3') \<longrightarrow> True)"
      ])
  case (1 uu uv uw ux uy uz g)
  then show ?case by simp
...
next
 case (28 cp i\<^sub>p t\<^sub>p pl ex el e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g)
  ...
  proof(intros)
    (*cd is the source calldata for a calldata copy
        and v,t is the pointer source and the source type*)
    fix ev cda k m g' id tp l
    assume as1:"load cp ((i\<^sub>p, t\<^sub>p) # pl) (ex # el) e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g = Normal ((ev, cda, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l)"
    then have none:"denvalue e\<^sub>v' $$ i\<^sub>p = None" using load.simps(1)[of cp i\<^sub>p t\<^sub>p  pl ex  el e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g] as1 by (simp split:Type.splits option.splits)
    ...
    then show "denvalue e\<^sub>v' $$ id = Some (Storage tp, l)" 
    proof(cases t\<^sub>p)
      case (Value x1)
      then have a40:"decl i\<^sub>p (Value x1) (Some (v, t)) cp cd (memory st) (storage st (address e\<^sub>v)) (cd', mem', sck', e\<^sub>v') = Some (c, m', k', e)" using a15 by simp

      have "\<forall>ev cda k m g' id x. load cp pl el  e c k' m' e\<^sub>v cd st g4' = Normal ((ev, cda, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l) \<longrightarrow> denvalue e $$ id = Some (Storage tp, l)"
        using 28(2)[OF Value none a10] a20 a7 a8 a10 a15 none by fastforce
      then have a50:"denvalue e $$ id = Some (Storage tp, l)" using a20 as1 by simp

      then have "\<exists>t l. denvalue e $$ i\<^sub>p = (Some (Value t, Stackloc l))" 
        using a15 a50 decl_env_kval[of i\<^sub>p x1 "(Some (v, t))" cp cd "(memory st)" "(storage st (address e\<^sub>v))" ] Value 
        using none by blast
      then have "i\<^sub>p \<noteq> id" using a50 by auto
      then have "denvalue e $$ id = denvalue e\<^sub>v' $$ id" using a40 a50 as1  decl_env_not_i by simp

      then show ?thesis using a50 by simp      
    next
      case (Calldata x2)
      then have "\<forall>ev cda k m g' id x. load cp pl el  e c k' m' e\<^sub>v cd st g4' = Normal ((ev, cda, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l) \<longrightarrow> denvalue e $$ id = Some (Storage tp, l)"
        using 28(4)[OF Calldata none a10] a20 a7 a8 a10 a15 none by fastforce
      then have a50:"denvalue e $$ id = Some (Storage tp, l)" using a20 as1 by simp

      then have a80:"\<forall>t l. denvalue e $$ i\<^sub>p = Some (t, l) \<longrightarrow> (\<forall>stl tp. l \<noteq> Storeloc stl \<and> t \<noteq> Storage tp)" 
        using  a50 decl_env_storage[OF a15] Calldata none by simp
      have "i\<^sub>p \<noteq> id" using a50 a80  by blast
      then have "denvalue e $$ id = denvalue e\<^sub>v' $$ id" using a50 as1 decl_env a15 decl_env_not_i by auto
      then show ?thesis using decl_env_storage a20 as1 a15 Calldata decl_env a15 a50 by presburger
    next
      case (Memory x3)
       then have "\<forall>ev cda k m g' id x. load cp pl el  e c k' m' e\<^sub>v cd st g4' = Normal ((ev, cda, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l) \<longrightarrow> denvalue e $$ id = Some (Storage tp, l)"
        using 28(6)[OF Memory none a10] a20 a7 a8 a10 a15 none by fastforce
      then have a50:"denvalue e $$ id = Some (Storage tp, l)" using a20 as1 by simp

      then have a80:"\<forall>t l. denvalue e $$ i\<^sub>p = Some (t, l) \<longrightarrow> (\<forall>stl tp. l \<noteq> Storeloc stl \<and> t \<noteq> Storage tp)" 
        using  a50 decl_env_storage[OF a15] Memory none by simp
      have "i\<^sub>p \<noteq> id" using a50 a80  by blast
      then have "denvalue e $$ id = denvalue e\<^sub>v' $$ id" using a50 as1 decl_env_not_i a15 by auto
      then show ?thesis using decl_env_storage a20 as1 a15 Memory decl_env a15 a50 by presburger
    next
      case (Storage x4)
      then show ?thesis using as1 load.simps by simp
    qed
...
next
  ...
  show ?case
  proof(intros)
    fix ev cd k m g' locs id x tp l
    assume a1: "load vt [] (vu # vv) vw vx vy vz wa wb wc g = Normal ((ev, cd, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l)"
    then show "denvalue vw $$ id = Some (Storage tp, l)" using load.simps(2) by (auto split:if_split_asm result.splits)
  qed
next
  case (30 wd we wf wg wh wi wj wk wl wm g)
  show ?case
  proof(intros)
    fix ev cd k m g' locs id x tp l
    assume a1: " load wd (we # wf) [] wg wh wi wj wk wl wm g = Normal ((ev, cd, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l)"
    then show "denvalue wg $$ id = Some (Storage tp, l)" using load.simps(3) by (auto split:if_split_asm result.splits)
...
next
  ...
  show ?case
  proof(intros)
    fix ev cda k m g' locs id x tp l
      (* cd' = cda \<Longrightarrow> sck' = k \<Longrightarrow> mem' = m*)
    assume a1: " load wn [] [] e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g = Normal ((ev, cda, k, m), g') \<and> denvalue ev $$ id = Some (Storage tp, l)"
    then show "denvalue e\<^sub>v' $$ id = Some (Storage tp, l)" using load.simps(4)[of wn e\<^sub>v' cd' sck' mem' e\<^sub>v cd st g] by (auto split:if_split_asm result.splits)
  qed
next
  case (32 i e cd st g)
  then show ?case by simp
