...
(*
ShowL\<^sub>i\<^sub>n\<^sub>t (ReadL\<^sub>i\<^sub>n\<^sub>t v) = v
Neccessary as ShowL\<^sub>i\<^sub>n\<^sub>t (ReadL\<^sub>i\<^sub>n\<^sub>t v) = v is not held with leading zeros
*)
...
(*^POTENTIAL CHECK: (if (size v = 42 \<and> ((String.explode v !1) = CHR ''x'')) then True else False)*)
