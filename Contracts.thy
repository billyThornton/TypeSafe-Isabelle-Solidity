...
(*
  Fixme: Merge accounts and storage to become
    Address \<Rightarrow> (account * StorageT)
*)
record State =   
  accounts :: Accounts
  stack :: Stack
...
next
  ...
  proof (rule allI[OF impI])
    ...
    with * ** *** show "address e' = address e \<and> sender e' = sender e \<and> svalue e' = svalue e \<and> contract e' = contract e" using init_address init_sender init_svalue init_contract by metis
  qed
qed
...
(*
  Fixme for Isabelle 23: moreover have "ffold (init ct) e {||} = e" using FSet.comp_fun_commute.ffold_empty[OF init_commte, of ct e] by simp
*)
...
(*
  TODO: Should we merge this with ffold_init_fmap?
*)
...
lemma ffold_init_emptyDen_ran:
  shows "fmlookup (denvalue (ffold (init ct) (emptyEnv a b c d) xs)) i = Some (Storage tp, Storeloc i)
...
next
 ...
 then have *: "ffold (init ct) (emptyEnv a b c d) (finsert x xs) =
  ...
  next
    ...
    show ?thesis
    proof intros
      assume **:"denvalue (ffold (init ct) (emptyEnv a b c d) (finsert x xs)) $$ i = Some (Storage tp, Storeloc i)"
      then have "denvalue (init ct x (ffold (init ct) (emptyEnv a b c d) xs)) $$ i = Some (Storage tp, Storeloc i)" using * by simp
