...
proof
  ...
  proof (cases "ReadL\<^sub>i\<^sub>n\<^sub>t val \<ge> 0")
    ...
    proof (cases "v < 2^256")
      ...
      with t2 \<open>v \<ge>0\<close> show ?thesis using assms Read_ShowL_id by auto
    next
      case False
...
proof
  ...
  proof (cases "ReadL\<^sub>i\<^sub>n\<^sub>t val \<ge> 0")
    ...
    proof (cases "v \<ge> 0")
      ...
      with t2 \<open>v < 2 ^ 256\<close> show ?thesis using assms Read_ShowL_id by auto
    next
      case False
...
(*
lemma balance_limit:
  shows "\<forall>ad. ReadL\<^sub>i\<^sub>n\<^sub>t (bal (acc ad)) \<ge> 0 \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (bal (acc ad)) < 2 ^ 256" by
*)
